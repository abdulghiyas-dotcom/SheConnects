            ...
            <div key={f.slug} className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm hover:shadow-md transition">
              <div className="flex flex-wrap gap-2 mb-4">
                {f.categories.map((cat) => (
                  <span key={cat} className="px-3 py-1 text-xs font-semibold bg-violet-50 text-violet-700 rounded-full">{cat}</span>
                ))}
              </div>
              <h3 className="text-xl font-semibold tracking-tight mb-3">{f.name}</h3>
              <p className="text-sm text-slate-600 leading-relaxed line-clamp-4">
                {f[language].bio}
              </p>
              <Link href={`/freelancers/${f.slug}`} className="mt-8 inline-block rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-500 px-6 py-2.5 text-sm font-medium text-white shadow-lg hover:opacity-95 transition">
