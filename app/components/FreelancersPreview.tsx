    ...
    <section id="freelancers" className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
      <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href={`/${language}/freelancers`} className="rounded-full border border-slate-200 bg-white/70 px-4 py-2 text-xs font-medium text-slate-700">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        {preview.map((f) => (
          <article key={f.slug} className="rounded-2xl border border-violet-50 bg-white p-5 shadow-sm">
            ...
            <Link href={`/${language}/freelancers/${f.slug}`} className="mt-4 inline-flex text-sm font-semibold text-violet-700">
